...
feedRouter.get("/", optionalAuth, async (req, res) => {
  try {
    ...
    const followingIdsSql = sql.join(safeFollowingIds.map(id => sql`${id}`), sql`, `);
    const isFollowingBonus = sql`CASE WHEN ${posts.userId} IN (${followingIdsSql}) THEN ${ALGO_CONFIG.FOLLOWING_BONUS} ELSE 0 END`;
    const isOwnPostBonus = sql`CASE WHEN ${posts.userId} = ${currentUserId} THEN ${ALGO_CONFIG.OWN_POST_BONUS} ELSE 0 END`;
    ...
    .where(
      and(
        ...
        eq(posts.moderationStatus, 'APPROVED'), // Community gönderilerini ana akıştan gizle
        eq(posts.moderationStatus, 'APPROVED'),
        sql`${posts.createdAt} >= NOW() - INTERVAL '30 days'`, // Performans için 30 gün limiti (Full Table Scan engelleme)
        or(
